...
export default function AdminProcesVerbauxPage() {
  ...
  const [editItem, setEditItem] = useState<ProcesVerbal | null>(null);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      await createProcesVerbal(formData);
      setOpen(false);
      resetForm();
      await load();
    } catch (e) {
  ...
  const handleEdit = (item: ProcesVerbal) => {
    setEditItem(item);
    setTitre(item.titre ?? "");
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await editProcesVerbal(Number(editItem.id), formData);
      setEditOpen(false);
      resetForm();
      await load();
    } catch (e) {
  ...
  const handleDelete = async (id: number | string) => {
    if (!confirm("Supprimer ce procès-verbal ?")) return;
    try {
      await deleteProcesVerbal(Number(id));
      await load();
    } catch (e) {
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Liste des procès-verbaux publiés après chaque réunion.
          </p>
        </div>
              ...
              <CardContent className="space-y-3 text-sm">
                ...
                {p.fichier && (
                  <div className="flex items-center gap-2 p-2 bg-gray-50 rounded-lg shadow-sm hover:bg-gray-100 transition">
                    ...
                    >
                    Télécharger le proces verbal (PDF)
                    </a>
                  </div>
                )}
